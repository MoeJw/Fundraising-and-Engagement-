...
{
	[ComVisible(true)]
	public class CavvPurchase : Transaction
	{
		...
		public CavvPurchase(Hashtable cavv_purchase)
			: base(cavv_purchase, xmlTags)
		{
		}

		public CavvPurchase(string order_id, string amount, string pan, string expdate, string cavv)
			: base(xmlTags)
		{
		...
		public CavvPurchase(string order_id, string cust_id, string amount, string pan, string expdate, string cavv)
			: base(xmlTags)
		{
			transactionParams.Add("order_id", order_id);
		...
		{
			string str = ((!country.Equals("US")) ? "cavv_purchase" : "us_cavv_purchase");
