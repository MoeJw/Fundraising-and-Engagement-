...
namespace Moneris
{
	...
	public class PreAuth : Transaction
	{
		...
		public PreAuth(Hashtable preauth)
			: base(preauth, xmlTags)
		{
		}

		public PreAuth(string order_id, string amount, string pan, string expdate, string crypt_type)
			: base(xmlTags)
		{
		...
		public PreAuth(string order_id, string cust_id, string amount, string pan, string expdate, string crypt_type)
			: base(xmlTags)
		{
			transactionParams.Add("order_id", order_id);
		...
		{
			string str = ((!country.Equals("US")) ? "preauth" : "us_preauth");
